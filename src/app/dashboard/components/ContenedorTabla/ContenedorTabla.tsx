...
export function ContenedorTabla(): React.JSX.Element {
  ...
  const { data: dataTransactions, isLoading: loadingTransactions } = useGetTransactions(tabs[tab].type);
  ...
  let rows = useMemo(() => {
    ...
    return rowsMap
  }, [dataTransactions, loadingTransactions])
        ...
        <Grid container xs={12}>{/*Contenedor Tabs y tabla */}
          <Grid item xs={12}>
            <Box sx={{ width: '100%' }}>
              ...
              {tabs.map((elm) => (
                <CustomTabPanel value={tab} index={elm.value}>
                  <Stack padding={"24px"}>
                  <Skeleton 
                  variant="rounded" 
                  height={"658px"} />
